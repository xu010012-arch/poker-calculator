...
with st.sidebar:
    ...
    st.divider()
    st.caption("该系统使用蒙特卡洛模拟算法，通过数万次随机模拟来预测最终胜率。")
    ...
    if not all([h1r, h1s, h2r, h2s]):
        ...
        if len(set(h+b)) != len(h+b):
            ...
            with st.spinner('AI 正在模拟数万种发牌可能...'):
                ...
                # 计算底池赔率门槛与期望值 (EV)
                # $$Pot\ Odds = \frac{Call}{Pot + Call}$$
                # $$EV = (Win\% \times Pot) - (Loss\% \times Call)$$
                ...
                if win_rate > pot_odds:
                    ...
                    st.success("**✅ 建议：跟注 (EV+)**")
                    st.markdown(f"""
                    * **数学依据**：胜率领先赔率要求 **{advantage:.1f}%**。从长远看，此举是盈利的。
                    * **操作策略**：如果对手风格偏软，可以考虑通过加注进一步建立底池压力。
                    """)
                else:
                    disadv = (pot_odds - win_rate) * 100
                    st.warning("**❌ 建议：弃牌 (EV-)**")
                    st.markdown(f"""
                    * **数学依据**：胜率低于赔率门槛 **{disadv:.1f}%**。投入的风险成本高于预期回报。
                    * **操作策略**：除非你有极强的诈唬把握或巨大的隐含赔率，否则应保留筹码。
                    """)
