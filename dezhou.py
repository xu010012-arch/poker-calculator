...
# CSS 注入：优化手机端显示，防止按钮错位，美化 Metric
st.markdown("""
    <style>
...
with st.sidebar:
    ...
    st.divider()
    st.caption("该系统使用蒙特卡洛模拟算法。这与你研究降雨预测时使用的随机森林 (Random Forest) 或 SVR 模型在处理不确定性数据时有相似的逻辑背景。")
        ...
        if len(set(h+b)) != len(h+b):
            ...
            with st.spinner('AI 正在模拟数万种发牌可能...'):
                ...
                if win_rate > pot_odds:
                    ...
                    * **操作策略**：从长远看，此举是盈利的。如果对手风格偏软，可以考虑通过加注进一步建立压力。
                    """)
                else:
                ...
                with st.expander("📊 查看详细牌型分布统计"):
                    ...
                    # 使用 .get() 安全获取键值，修复 KeyError
                    prob_list = []
                    for k, v in hand_probs.items():
                        if v > 0:
                            hand_name = CLASS_MAP.get(k, f"其他牌型({k})")
                            prob_list.append({"牌型": hand_name, "概率": f"{v*100:.1f}%"})
